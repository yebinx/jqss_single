...
export class CompScrollView extends ScrollView {
    ...
    _onTouchMoved1(event: EventTouch, captureListeners?: Node[]) {
        ...
        const wheelPrecision = -2;
        const scrollY = event.getUIDelta().y / (this.verticalScrollBar.node.getComponent(UITransform).height - this.verticalScrollBar.handle.getComponent(UITransform).height) * this._content.getComponent(UITransform).height
        if (this.vertical) {
