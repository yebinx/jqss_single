...
import { CustomDateSelectList } from './CustomDateSelectList';
...
@ccclass('RecordCustomDate')
export class RecordCustomDate extends Component {
    ...
    private selectList: CustomDateSelectList = null;
    ...
    private onBtnSuccess() {
        ...
        this.node.destroy();
        this.record.emit(PUBLIC_EVENTS.REQUEST_CUSTOM_DATE_RECORD[PUBLIC_EVENTS.REQUEST_CUSTOM_DATE_RECORD], startTimestamp, endTimestamp, SelectDateType.CUSTOM)
    }
    ...
    private getSelectList(target: Node, szData: number[], currentDate: number, callback: (selectDate: number) => void) {
        if (!isValid(this.selectList)) {
            ...
            this.selectList = nd.getComponent(CustomDateSelectList);
        }
